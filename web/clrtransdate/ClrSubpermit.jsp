...
//定義變數
int pageRows = 100;
String tblname = "labdyn_subpermit";
...
String qprmtdate = AbString.rtrimCheck(request.getParameter("qprmtdate"));
        ...
        <center>
            <form action ="<%=thisPage%>" method="post">
                ...
                <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="500">
                    ...
                    <tr >
                        <td width=25% align=right >函號：</td>
                        <td width=25% ><input type="text" name="qprmtno" value="<%=qprmtno%>" size="12"></td>
                        <td width=25% align=right >補發日期：</td>
                        <td width=25% ><input type="text" name="qprmtdate" value="<%=qprmtdate%>" size="12"><br>（yyyymmdd）</td>
                    </tr>
                    <tr >
...
if (qprmtdate.length() > 0) srch += " and prmtdate = " + AbSql.getEqualStr(qprmtdate);
...
if (srch.length() > 0) {
    ...
    //寫入日誌檔
    String srchdata = "補發函資料（.rep檔 - subpermit）";
    if (qprmtno.length() > 0) srchdata += "，函號：" + qprmtno;
    if (qprmtdate.length() > 0) srchdata += "，補發日期：" + qprmtdate;
...
%>
    ...
    <form action ="<%=thisPage%>" method="post" onsubmit="return confirm('確定要清除傳輸日期欄位嗎？');">
        ...
        <input type="hidden" name="qprmtdate" value="<%=qprmtdate%>">

        <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="548">
            <tr bgcolor="#FF9900">
            ...
            <tr bgcolor="#FF9900">
                ...
                <td width=100 align="center">函號</td>
                <td width=100 align="center">補發日期</td>
                <td width=100 align="center">招募期限</td>
                <td width=100 align="center">補發原因</td>
            </tr>
<%
    //顯示資料
    qs = "select" + (sqlFirstCmd.length() > 0 ? sqlFirstCmd + (p*pageRows) : "") + " rowid, m.* from " + tblname + " m " + srch
            + " order by prmtno,prmtdate";
    ...
    while (rs.next() && (cnt < pageRows)) {
        ...
        String repreason = AbString.rtrimCheck(rs.getString("repreason"));
...
%>
            ...
            <tr>
                ...
                <td align="center"><%=strCheckNullHtml(repreason)%></td>
            </tr>

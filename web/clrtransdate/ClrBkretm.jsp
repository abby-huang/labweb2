...
<%
String pageHeader = "遞補資料重傳";
request.setCharacterEncoding("UTF-8");

//檢查 CSRF
String thisPage = request.getRequestURI();
String referer = new java.net.URI(request.getHeader("referer")).getPath();
String CSRFToken = AbString.rtrimCheck((String)session.getAttribute("CSRFToken"));
String CSRFToken2 = AbString.rtrimCheck(request.getParameter("CSRFToken"));
if (thisPage.equals(referer) && CSRFToken.equals(CSRFToken2)) {
...
String qbkretno = AbString.rtrimCheck(request.getParameter("qbkretno")).toUpperCase();
        ...
        <center>
            <form action ="" method="post">
                ...
                <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="500">
                    ...
                    <tr >
                        <td width=25% align=right>遞補函文號：</td>
                        <td width=25% colspan="3"><input type="text" name="qbkretno" value="<%=qbkretno%>" size="12"></td>
                    </tr>
                    <tr >
...
//限制條件
String srch = "";
if (qbkretno.length() > 0) srch += " and bkretno = " + AbSql.getEqualStr(qbkretno);
if (qnatcode.length() > 0) srch += " and labncode = " + AbSql.getEqualStr(qnatcode);
if (qpassno.length() > 0) srch += " and visanum = " + AbSql.getEqualStr(qpassno);
...
if (srch.length() > 0) {
    ...
    //寫入日誌檔
    String srchdata = "遞補資料（.sub檔 - bkretm）";
    if (qbkretno.length() > 0) srchdata += "，遞補函文號：" + qbkretno;
...
%>
    ...
    <form action ="" method="post" onsubmit="return confirm('確定要清除傳輸日期欄位嗎？');">
        <input type="hidden" name="qbkretno" value="<%=qbkretno%>">
        ...
        <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="756">
            ...
            <tr bgcolor="#FF9900">
                ...
                <td width=100 align="center">遞補函文號</td>
                <td width=100 align="center">國籍代碼</td>
                <td width=100 align="center">護照號碼</td>
                <td width=100 align="center">核准招募文號</td>
                <td width=100 align="center">核准函日期</td>
                <td width=100 align="center">遞補招募期限日期</td>
            </tr>
<%
    //顯示資料
    qs = "select" + (sqlFirstCmd.length() > 0 ? sqlFirstCmd + (p*pageRows) : "") + " rowid, m.* from " + tblname + " m " + srch
            + " order by labncode,visanum";
    ...
    while (rs.next() && (cnt < pageRows)) {
        ...
        String natcode = AbString.rtrimCheck(rs.getString("labncode"));
        String passno = strCheckNull(AbString.rtrim(rs.getString("visanum")));
        ...
        String visaperiod = AbString.rtrimCheck(rs.getString("visaperiod"));
...
%>
            ...
            <tr>
                ...
                <td align="center"><%=strCheckNullHtml(bkretno)%></td>
                <td align="center"><%=strCheckNullHtml(natcode)%></td>
                <td align="center"><%=strCheckNullHtml(passno)%></td>
                <td align="center"><%=strCheckNullHtml(prmtno)%></td>
                <td align="center"><%=strCheckNullHtml(prmtdate)%></td>
                <td align="center"><%=strCheckNullHtml(visaperiod)%></td>
            </tr>

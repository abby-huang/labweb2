...
//定義變數
int pageRows = 100;
String tblname = "labdyn_healthchk";
...
String qchktype = AbString.rtrimCheck(request.getParameter("qchktype")).toUpperCase();
String qchkdate = AbString.rtrimCheck(request.getParameter("qchkdate"));
        ...
        <center>
            <form action ="<%=thisPage%>" method="post">
                ...
                <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="500">
                    ...
                    <tr >
                        <td width=25% align=right >體檢日期：</td>
                        <td width=75% colspan="3"><input type="text" name="qchkdate" value="<%=qchkdate%>" size="12">（yyyymmdd）</td>
                    </tr>
                    <tr >
...
//if (qchktype.length() > 0) srch += " and chktype = " + AbSql.getEqualStr(qchktype);
if (qchkdate.length() > 0) srch += " and chkdate = " + AbSql.getEqualStr(qchkdate);
if (srch.length() > 0) srch = " where " + srch.substring(4);

//查詢動作 - 顯示資料
if (srch.length() > 0) {
    srch += " and (chktype = '0' or chktype = 'A')";

    //頁數
    int p = 1;

    //寫入日誌檔
    String srchdata = "外勞初次體檢資料（.hea檔 - healthchk）";
    if (qnatcode.length() > 0) srchdata += "，國籍代碼：" + qnatcode;
    if (qpassno.length() > 0) srchdata += "，護照號碼：" + qpassno;
    if (qchktype.length() > 0) srchdata += "，體檢種類代碼：" + qchktype;
    if (qchkdate.length() > 0) srchdata += "，體檢日期：" + qchkdate;
...
%>
    ...
    <form action ="<%=thisPage%>" method="post" onsubmit="return confirm('確定要清除傳輸日期欄位嗎？');">
        ...
        <input type="hidden" name="qchktype" value="<%=qchktype%>">
        <input type="hidden" name="qchkdate" value="<%=qchkdate%>">

        <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="676">
            <tr bgcolor="#FF9900">
            ...
            <tr bgcolor="#FF9900">
                ...
                <td width=80 align="center">體檢種類</td>
                <td width=80 align="center">體檢日期</td>
                <td width=80 align="center">總結果</td>
                <td width=80 align="center">體檢報告日</td>
            </tr>
<%
    //顯示資料
    ...
    while (rs.next() && (cnt < pageRows)) {
        ...
        String chktype = strCheckNull(AbString.rtrim(rs.getString("chktype")));
        String chkdate = strCheckNull(AbString.rtrim(rs.getString("chkdate")));
        String chkresult = strCheckNull(AbString.rtrim(rs.getString("chkresult")));
        String rptdate = strCheckNull(AbString.rtrim(rs.getString("rptdate")));
...
%>
            ...
            <tr>
                ...
                <td align="center"><%=strCheckNullHtml(chktype)%></td>
                <td align="center"><%=strCheckNullHtml(chkdate)%></td>
                <td align="center"><%=strCheckNullHtml(chkresult)%></td>
                <td align="center"><%=strCheckNullHtml(rptdate)%></td>
            </tr>

...
String qagentcode = AbString.rtrimCheck(request.getParameter("qagentcode")).toUpperCase();
            ...
            <form action ="<%=thisPage%>" method="post">
                ...
                <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="500">
                    ...
                    <tr >
                        ...
                        <td width=25% align=right >公司編號：</td>
                        <td width=25% ><input type="text" name="qagentcode" value="<%=qagentcode%>" size="12"></td>
                    </tr>
                    <tr >
...
if (qagentcode.length() > 0) srch += " and agentcode = " + AbSql.getEqualStr(qagentcode);
...
if (srch.length() > 0) {
    ...
    //寫入日誌檔
    String srchdata = "國外仲介公司資料（.fag檔 - fagent）";
    if (qnatcode.length() > 0) srchdata += "，國籍代碼：" + qnatcode;
    if (qagentcode.length() > 0) srchdata += "，公司編號：" + qagentcode;
...
%>
    ...
    <form action ="<%=thisPage%>" method="post" onsubmit="return confirm('確定要清除傳輸日期欄位嗎？');">
        ...
        <input type="hidden" name="qagentcode" value="<%=qagentcode%>">
        ...
        <table border=1 bgcolor="#F8BE67" bordercolor="#FF9900" width="652">
            ...
            <tr bgcolor="#FF9900">
                ...
                <td width=100 align="center">公司編號</td>
                <td width=100 align="center">公司名稱</td>
                <td width=100 align="center">認可起始日</td>
                <td width=100 align="center">認可終止日</td>
            </tr>
<%
    //顯示資料
    qs = "select" + (sqlFirstCmd.length() > 0 ? sqlFirstCmd + (p*pageRows) : "") + " rowid, m.* from " + tblname + " m " + srch
            + " order by natcode,agentcode";
    ...
    while (rs.next() && (cnt < pageRows)) {
        ...
        String agentcode = strCheckNull(AbString.rtrim(rs.getString("agentcode")));
        String agentname = strCheckNull(AbString.rtrim(rs.getString("agentname")));
        String prmtbdate = strCheckNull(AbString.rtrim(rs.getString("prmtbdate")));
        String prmtedate = strCheckNull(AbString.rtrim(rs.getString("prmtedate")));
...
%>
            ...
            <tr>
                ...
                <td align="center"><%=strCheckNullHtml(agentcode)%></td>
                <td align="center"><%=strCheckNullHtml(agentname)%></td>
                <td align="center"><%=strCheckNullHtml(prmtbdate)%></td>
                <td align="center"><%=strCheckNullHtml(prmtedate)%></td>
            </tr>

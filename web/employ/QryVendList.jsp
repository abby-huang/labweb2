...
session.setAttribute("debugMsg", thisPage + " - " + QryVendSql);

//計算筆數
qs = "select count(distinct(m.vendno || m.wkadseq)) from labdyn_vend m "
    + QryVendSql;
    //+ " order by m.regno";
...
<table border=0 width=1000>
    ...
    <form action="QryVendListPrint.jsp" target="_blank">
    <td align=left width=5%>
    ...
    <form action="QryVendListCsv.jsp">
    <td align=left width=5%>
...
while (rs.next() && (cnt < pmax)) {
    ...
    String StatiUrl = "&nbsp;";
    if (regno.matches("[0-9].*")) StatiUrl = "<a HREF='QryVendStati.jsp?regno=" + regno + "&wkadseq=" + wkadseq + "'>附加案/5級制外勞人數</a>";
%>



<tr>
    <td align=center><a HREF="QryVendLaborListCsv.jsp?regno=<%=regno%>&wkadseq=<%=wkadseq%>">清冊下載</a></td>
    <td align=center><a HREF="QryVendLaborMain.jsp?regno=<%=regno%>&wkadseq=<%=wkadseq%>"><%=regno%><%=wkadseq%></a></td>

...
if (method.equalsIgnoreCase("GET")) {
    //簡列資料
    String command = AbString.rtrimCheck( request.getParameter("command") );
...
//刪除
    ...
    JSONObject jsonMain = new JSONObject();
    //刪除資料
    stmt.executeUpdate("delete from " + mainTable + " where id=" + AbSql.getEqualStr(id));
    errId = "0";
    errMsg = "成功刪除資料：" + id;
...
//新增、修改
    ...
    //檢查新增代碼
    if ((errMsg.length() == 0) && (method.equalsIgnoreCase("POST"))) {
        rs = stmt.executeQuery("select id from " + mainTable + " where id=" + AbSql.getEqualStr(id));
        if (rs.next()) {
            errId = "1";
            errMsg = "代碼已經存在 "+id;
            invalidField="id";
